# Python code for Multiple Color Detection and move 
...
webcam = cv2.VideoCapture(Camera.video_source)
...
while(1):
    ...
    contours = cv2.findContours(blue_mask,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
    for pic, contour in enumerate(contours):
        area = cv2.contourArea(contour)
        if(area > 300):
            x, y, w, h = cv2.boundingRect(contour)
